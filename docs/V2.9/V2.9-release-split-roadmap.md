...
V2.9 主发布拆分为 `V2.91` 到 `V2.96` 六个小版本。`V2.97` 作为发布后的竞品追平路线图，承接 Supermemory / mem0 的实际功能差距，不打断 V2.96 的发布门禁。
...
| `V2.97` | Supermemory / mem0 实际追平路线 | 从实际功能差距出发，补 connectors、graph、namespace、profile、SDK、多模态 | 最全追平路线图、执行切片、验收门槛 |
...
## 9. V2.97 Supermemory / mem0 实际功能追平

### 9.1 目标

V2.97 基于真实功能重新对比 Meat Memory V2.9、Supermemory 和 mem0，不再只做能力映射。目标是把差距拆成可执行路线图，让 V2.9 之后的版本能有节奏地追平并超过两家。

### 9.2 范围

- Supermemory 对标：connectors、profile、Memory Router、网页 / 文件 / 多模态 ingest、开发者接入体验。
- mem0 对标：Graph Memory、user / agent / app / run / org namespace、SDK、自托管 server、event/webhook。
- Meat Memory 超越项：Recall Trace Studio、Memory Health Ops、Policy DSL、Encrypted Passport、public benchmark pack。
- 输出 `V2.97-A` 到 `V2.97-F` 执行切片。

### 9.3 不做

- 不把 V2.97 混进 V2.96 发布门禁。
- 不再接受只有 mapping fixture 的“追平”定义，必须有真实 connector、adapter、SDK 或 UI demo。
- 不以牺牲 V2.9 的 trace、health、passport、governance 优势换取接入速度。

### 9.4 任务范围

| 范围 | 任务 |
|---|---|
| 竞品 | `V2.9-COMPET-005`、`V2.9-COMPET-006` |
| Connector | `V2.9-CONN-004`、`V2.9-CONN-005`、`V2.9-CONN-006`、`V2.9-CONN-007` |
| Graph | `V2.9-GRAPH-001`、`V2.9-GRAPH-002`、`V2.9-GRAPH-003` |
| Namespace | `V2.9-NS-001`、`V2.9-NS-002` |
| SDK / Router | `V2.9-SDK-001`、`V2.9-SDK-002`、`V2.9-SDK-003` |
| Multimodal | `V2.9-MM-001`、`V2.9-MM-002`、`V2.9-MM-003` |
| 超越项 | `V2.9-OPS-001`、`V2.9-POLICY-001`、`V2.9-PASS-005` |

### 9.5 验收

- 每个 connector 支持 dry-run、增量 sync、source refs、failure report。
- Graph Memory 能在 search payload 中返回 relation path explanation。
- Namespace API 覆盖 user / agent / app / run / org / project。
- TypeScript / Python SDK 覆盖 remember、search、profile、trace、passport、connector 管理。
- mem0 / Supermemory import adapter 支持 dry-run、scope mapping、id mapping、validation error report。
- 多模态 ingest 输出 evidence span，而不是只保存 caption 文本。
- V2.97 新增生产代码 targeted coverage 继续保持 100%。
...
flowchart LR
  ...
  V297["V2.97 Supermemory / mem0 Parity Roadmap"]
...
`V2.91` 是所有后续小版本的底座，因为 benchmark 和兼容契约决定后续功能是否可证明、是否不破坏旧行为。`V2.96` 是发布收口，不应承载新的高风险核心语义。`V2.97` 是发布后的追平路线图，允许规划更完整的 connectors、graph、SDK 和多模态能力。
...
- `V2.97` 路线图明确 Supermemory / mem0 的实际功能差距、执行切片和真实功能验收标准。
